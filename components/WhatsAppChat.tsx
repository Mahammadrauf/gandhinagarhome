...
const NUDGE_INTERVAL_MS = 30000; // every 30s (your requirement)
const NUDGE_AUTOHIDE_MS = 2600;
const SNOOZE_AFTER_CLOSE_MS = 180000; // 3 min snooze after closing popup
const STORAGE_KEYS = {
...
const WhatsAppChat: React.FC = () => {
  ...
  const toggleOpen = (next?: boolean) => {
    ...
    setIsOpen(willOpen);
    if (!willOpen) {
      // snooze nudges for a bit after closing modal
  ...
  /** Nudges every 30s when closed; first after 3s; auto-hide after ~2.6s */
  useEffect(() => {
    const clearTimers = () => {
    ...
    if (isOpen) return; // don't show nudges while open
    ...
    intervalRef.current = setInterval(() => {
      // If user interacted recently, skip one cycle (less annoying)
  ...
  /** Pause nudges for a bit when user scrolls/hover the button */
  useEffect(() => {
    const onUserAction = () => {
  ...
  /** Close on escape key, focus trap basics */
  useEffect(() => {
    if (!isOpen) return;
    ...
    <>
      ...
      <div className="fixed bottom-6 right-6 z-50 flex flex-col items-end">
        ...
        <button
          ...
          className={[
            "relative bg-[#25D366] hover:bg-[#20BA5A] text-white rounded-full p-4 shadow-2xl",
            "transition-all transform w-16 h-16 md:w-20 md:h-20 flex items-center justify-center",
        ...
        >
          {/* unread dot */}
          {unread && (
            <span className="absolute -top-1 -right-1 w-4 h-4 rounded-full bg-red-500 ring-2 ring-white" aria-hidden />
          )}
