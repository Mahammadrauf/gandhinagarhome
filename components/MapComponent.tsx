...
const loadLeaflet = async (): Promise<any> => {
  ...
  const existingJs = document.getElementById(
    LEAFLET_JS_ID
...
function InteractiveMap({
  ...
  const geocodeCacheRef = useRef<Map<string, LatLng | null>>(
    new Map()
  );
  ...
  useEffect(() => {
    ...
    const renderMarkers = async () => {
      ...
      for (const l of toGeocode) {
        ...
        marker.on("click", () => {
          const targetId =
            l.propertyId || String(l.id);
          onSelectProperty?.(targetId);
        });
        marker.bindPopup(
          `<div style="width:240px">
            <div style="font-weight:700; font-size:14px; color:#0f172a; margin-bottom:4px;">${String(
              l.title
            ).replace(/</g, "&lt;")}</div>
            <div style="font-size:11px; color:#64748b; margin-bottom:8px;">${String(
              l.locality
            ).replace(/</g, "&lt;")}, ${String(
            l.city
          ).replace(/</g, "&lt;")}</div>
            <div style="display:flex; align-items:center; justify-content:space-between; border-top:1px solid #f1f5f9; padding-top:8px;">
              <div style="font-weight:800; font-size:14px; color:#0f172a;">${String(
                l.priceLabel
              ).replace(/</g, "&lt;")}</div>
              <div style="font-size:11px; color:#64748b;">${
                l.bedrooms
              } Bed • ${l.areaSqft} sqft</div>
            </div>
          </div>`
        );
